...
- 每个标记开始一张 Cloze 笔记，正文持续到下一个同类标记或文件末尾；一篇笔记可以放多个标记来建立多张卡；
- 标记必须单独占一行，标记本身不会同步到 Anki；
- 区域内部的空行、标题、列表、引用、表格、公式、图片、代码块、Basic 分隔符、Choice 语法和三级标题都属于同一张 Cloze 的 `Content`；
- 代码围栏中的标记只是示例，不会启用显式模式；围栏中的 Cloze 不用于判断区域是否有效，也不产生阅读遮罩；
- 新标记前面的 Basic 和 Choice 仍正常解析；标记后的内容属于该 Cloze，直到下一个标记或文件末尾；
- 空区域或没有有效填空的区域会显示明确错误，不会退回整篇模式；
