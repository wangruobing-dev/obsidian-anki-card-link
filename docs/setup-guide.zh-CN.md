...
一张 Cloze 需要跨多个段落、标题、列表、引用、表格、公式、图片或代码块时，使用标准格式：
...
- 标签之间是一张 Cloze 笔记，一篇笔记可以有多个 Cloze 区域；
- 标签必须单独占一行、成对出现，不允许嵌套，标签不会同步到 Anki；
- 区域内部的空行、标题、列表、引用、表格、公式、图片、代码块、Basic 分隔符、Choice 语法和三级标题都属于同一张 Cloze 的 `Content`；
- 代码围栏中的边界标签只是示例，不会启用显式模式；围栏中的 Cloze 不用于判断区域是否有效，也不产生阅读遮罩；
- 文件中只要出现任意开始或结束标签，就进入显式模式：只有完整区域内的有效 Cloze 会同步，区域外 Cloze 被忽略，区域外 Basic 和 Choice 仍正常；
- 标签缺失、顺序错误、嵌套、空区域或无有效填空时会显示明确错误，不会退回整篇模式；
- 如果文件中完全没有标签，围栏代码块外只要有一个有效 Cloze，整篇正文就是一张兼容 Cloze；YAML Frontmatter、同步按钮、旧 UID 块和插件元数据不会进入 `Content`；
- 旧笔记继续可用，不强制迁移，也不会自动批量添加标签；混合 Basic、Choice 和 Cloze 时推荐始终使用边界标签。
...
有选中文字时，命令保留原 Markdown 并用标准标签包裹；没有选区时插入空区域，并把光标放在两个标签之间。光标已经在区域内、选区包含标签或与区域部分/完整重叠时，命令会拒绝插入，避免重复和嵌套。
