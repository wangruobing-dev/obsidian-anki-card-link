...
正文中的 `[来源](https://example.com/?app_platform=ios&app_version=1)` 会转换为 Anki 中可点击的链接。支持 HTTP/HTTPS 地址的查询参数、中文、转义符、成对括号、尖括号包裹的网址及可选标题；代码和公式中的链接写法保持原样。错误嵌套的链接和不支持的目标地址保留为文本。已有卡片再次同步后会更新旧的链接 HTML，不会自动清理源笔记中的错误链接或已有空行。

再次同步时使用按钮 URL 中的 noteId 调用 `notesInfo`，先核验笔记类型和配置的 URI 字段中的 UID，再更新发生变化的同步字段。开启按文件夹路径命名时，还会把该笔记的卡片移入对应牌组，即使内容没有变化；仅内容和牌组都无需更新时才跳过。noteId 不存在、笔记类型不一致或 UID 不一致时也会跳过，不会创建替代笔记。改变 Anki 牌组或移动 Obsidian 文件不影响匹配；更新成功后会刷新保存的来源路径。

### 知识库与文件夹牌组名称

开启“使用当前文件夹路径作为牌组名称”后，牌组格式为 `知识库名称::文件夹::子文件夹`。在 **设置 → Anki Card Link → 同步设置** 中，“知识库牌组名称”留空时自动使用当前知识库文件夹名，也可填写自定义名称，首尾空格会被忽略。

例如知识库文件夹名为 `Obsidian`，笔记 `生活/百科知识/区划代码.md` 会进入 `Obsidian::生活::百科知识`，根目录的 `首页.md` 直接进入 `Obsidian`。若自定义为“我的知识库”，则分别进入 `我的知识库::生活::百科知识` 和 `我的知识库`。

已有卡片在下次同步时移动，保留原笔记 ID、卡片 ID 和复习记录，包括同一 Cloze 笔记生成的全部卡片。重命名知识库、修改自定义名称、移动源文件，或在 Anki 中手动调整牌组后，下次同步都会按当前规则归组。自定义牌组名称不影响返回 Obsidian 链接中的真实知识库名称。旧空牌组会保留，不会自动迁移整个知识库。如果内容更新成功后移动失败，同步报告会显示失败，再次同步即可重试归组。

关闭按文件夹路径命名后，新建卡片使用“默认牌组名称”，已有卡片继续留在当前牌组。
...
- 知识库名称与文件夹路径映射为 Anki `::` 层级牌组，支持自定义知识库名称和再次同步时归组；
