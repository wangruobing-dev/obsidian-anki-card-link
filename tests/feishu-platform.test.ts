...
const YOUDAO_FILES = [
	'src/core/share-markdown.ts',
	'src/core/content-hash.ts',
	'src/services/youdao-api.ts',
	'src/services/youdao-sync-index.ts',
	'src/services/youdao-sync.ts',
];

describe('cloud sync platform compatibility', () => {
	it('uses browser and Obsidian APIs without Node or Electron runtime dependencies', async () => {
		if (!Platform.isDesktopApp) return;
		const { readFileSync } = await import('node:fs');
		const source = [...FEISHU_FILES, ...YOUDAO_FILES].map((path) => readFileSync(path, 'utf8')).join('\n');
